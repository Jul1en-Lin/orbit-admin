...
- [ ] 提供构建、开发启动、环境配置、统一检查、复制复用与部署说明；前后端分别构建与部署，不新增模板自动升级机制。
- [ ] 浏览器可见配置视为公开信息，不写入服务端秘密；实际凭据通过非仓库渠道提供，不重新提交已脱敏的连接信息。
- [ ] 记录实际执行命令与验证结果，区分构建通过、局部链路验证、真实联调与实际部署；真实网关可用且获准时核实登录、刷新会话与代理路径，不可用时标记未验收与阻碍；未获授权不更改共享基础设施、域名、证书或发布服务。
